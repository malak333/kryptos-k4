...
#[derive(Debug, Clone, Serialize, PartialEq)]
pub struct CiphertextWindowBalanceEvaluation {
    pub artifact_path: String,
    pub observation_id: Option<String>,
    pub observation_source_ids: Vec<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    pub observed_positions_one_based: Vec<usize>,
    pub window_balance_position_count: usize,
    pub window_balance_hits: usize,
    pub window_balance_hit_rate: f64,
    pub matching_window_balance_positions_one_based: Vec<usize>,
    pub non_window_balance_positions_one_based: Vec<usize>,
    pub null_mean_window_balance_hits: f64,
    pub null_std_dev_window_balance_hits: f64,
    pub empirical_p_value: f64,
    pub iterations: usize,
    pub seed: u64,
    pub promoted_candidate: bool,
...
pub fn evaluate_ciphertext_window_balance_positions(
    artifact_path: impl AsRef<std::path::Path>,
    observed_positions_one_based: Vec<usize>,
    iterations: usize,
    seed: u64,
) -> Result<CiphertextWindowBalanceEvaluation> {
    if observed_positions_one_based.is_empty() {
        bail!("ciphertext-window-balance evaluation requires at least one observed position");
    }
    if iterations == 0 {
        bail!("ciphertext-window-balance evaluation iterations must be greater than zero");
    }

    let artifact_path = artifact_path.as_ref();
    let artifact = std::fs::read_to_string(artifact_path)?;
    let prior: CiphertextWindowBalancePrior = serde_json::from_str(&artifact)?;
    if prior.artifact_kind != "ciphertext-window-balance-prior" {
        bail!(
    ...
    for position in &observed_positions_one_based {
        ...
        if !non_anchor_set.contains(position) {
            bail!(
                "observed positions must be one-based non-anchor K4 positions from the ciphertext-window-balance artifact"
            );
        }
    }

    let window_balance_set: HashSet<_> = prior
        .window_balance_positions
        .iter()
        .map(|position| position.position_one_based)
        .collect();
    let matching_window_balance_positions_one_based: Vec<_> = observed_positions_one_based
        .iter()
        .copied()
        .filter(|position| window_balance_set.contains(position))
        .collect();
    let non_window_balance_positions_one_based: Vec<_> = observed_positions_one_based
        .iter()
        .copied()
        .filter(|position| !window_balance_set.contains(position))
        .collect();
    let window_balance_hits = matching_window_balance_positions_one_based.len();
    let window_balance_hit_rate =
        window_balance_hits as f64 / observed_positions_one_based.len() as f64;
    ...
    let greater_or_equal = null_values
        .iter()
        .filter(|value| **value >= window_balance_hits as f64)
        .count();
    let empirical_p_value = (greater_or_equal as f64 + 1.0) / (iterations as f64 + 1.0);

    Ok(CiphertextWindowBalanceEvaluation {
        artifact_path: artifact_path.display().to_string(),
        observation_id: None,
        observation_source_ids: Vec::new(),
        observation_source_review_file: None,
        ...
        observed_position_count: observed_positions_one_based.len(),
        observed_positions_one_based,
        window_balance_position_count: prior.window_balance_position_count,
        window_balance_hits,
        window_balance_hit_rate,
        matching_window_balance_positions_one_based,
        non_window_balance_positions_one_based,
        null_mean_window_balance_hits: null_mean,
        null_std_dev_window_balance_hits: null_std_dev,
        empirical_p_value,
        iterations,
        seed,
        promoted_candidate: false,
        note: "Ciphertext window-balance evaluation scores source-backed non-anchor positions against a ciphertext-only local-window balance artifact with a seeded same-size position-set null; it is not a claimed solution.",
    })
}
