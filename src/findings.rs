...
pub fn findings() -> Vec<Finding> {
    vec![
        ...
        Finding {
            ...
            output_summary: "Ad hoc position lists now emit a diagnostic warning, source-backed observation files require preregistration-backed artifact validation before scoring, and archived evaluations re-run archive-local preregistration/artifact/observation checks.",
            baseline_comparison: "This is an evidence-boundary guard rather than a positive statistical signal; it prevents stale artifacts, ad hoc position lists, or internally tampered archives from being interpreted as independent support.",
        ...
        Finding {
            ...
            output_summary: "The lane-status command reports 13 lanes, 13 ready for source-backed observations, 0 invalid lanes, 13 prediction artifacts, 2 unique prediction artifact contents, 2 unique ready prediction artifact contents, 1 duplicate artifact group, period family 12 lanes/1 unique ready artifact, spacing family 1 lane/1 unique ready artifact, and promoted false.",
