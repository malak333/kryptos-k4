...
| `claim-verification-status` | Focused quarantine inventory for registered solution-claim sources, committed non-leaking verification archives, unarchived claim sources, per-source verifier/input-plan guidance, and the safe local verifier commands in Markdown or JSON. |
