...
| `validate-evaluation-archive` | Archive gate for independent observation evaluations, checking required files, local replay command paths, artifact kind, source-backed observation consistency, and no-promotion boundaries in Markdown or JSON. |
...
- **F7 Machine-readable gates protect future independent evidence (H3)**
  - Sources: `experiments/preregistrations/non-anchor-position-period-v2.json`, `experiments/predictions/non-anchor-position-period-v2.json`, `experiments/preregistrations/non-anchor-position-period-v3.json`, `experiments/predictions/non-anchor-position-period-v3.json`, `experiments/preregistrations/non-anchor-position-period-v4.json`, `experiments/predictions/non-anchor-position-period-v4.json`, `experiments/preregistrations/non-anchor-position-period-v5.json`, `experiments/predictions/non-anchor-position-period-v5.json`, `experiments/preregistrations/non-anchor-position-period-followup-v1.json`, `experiments/predictions/non-anchor-position-period-followup-v1.json`, `experiments/PROGRESS_LOG.md`, `release-check`
  - Steps: validate the follow-up, v2, v3, v4, and v5 preregistrations with JSON output; validate the committed follow-up, v2, v3, v4, and v5 prediction artifacts with JSON output; run release-check in JSON mode and require every local preflight gate to pass
  - Output: The follow-up, v2, v3, v4, and v5 preregistrations and artifacts validate as non-promotional, and release-check JSON reports every local gate as passed.
...
- preregistrations-valid: true (Committed non-template preregistrations validate. path=/Users/michaelnobile/Antigravity/Ciphers/experiments/preregistrations; checked=9)
- prediction-artifacts-valid: true (Committed prediction artifacts match their preregistrations and deterministic generators. path=/Users/michaelnobile/Antigravity/Ciphers/experiments/predictions; checked=9)
- independent-lanes-ready: true (Independent preregistered lanes are ready for source-backed observation files. path=/Users/michaelnobile/Antigravity/Ciphers/experiments/preregistrations; ready=9; checked=9)
