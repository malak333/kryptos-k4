...
fn source_frontier_classifies_all_registered_sources() {
    Command::cargo_bin("kryptos-k4")
        ...
        .stdout(predicate::str::contains("valid source-backed archives: 36"))
        .stdout(predicate::str::contains(
            "all source-backed archives negative: false",
        ))
        .stdout(predicate::str::contains(
        ...
        .stdout(predicate::str::contains(
            "Already used in 36 source-backed evaluation archives",
        ))
    ...
    assert_eq!(json["valid_source_backed_archive_count"], 36);
    assert_eq!(json["all_source_backed_archives_negative"], false);
    assert_eq!(json["source_backed_archive_correction_count"], 36);
    assert!(
    ...
    assert_eq!(cia_sculpture["source_backed_archive_count"], 36);
    assert!(
...
fn source_frontier_summary_prints_compact_next_action() {
    Command::cargo_bin("kryptos-k4")
        ...
        .stdout(predicate::str::contains(
            "already-scored source-backed archives: cia-sculpture (36 archived evaluations)",
        ))
        .stdout(predicate::str::contains(
    ...
    assert!(
        json["already_scored_source_backed_archives"][0]
            .as_str()
            .unwrap()
            .contains("cia-sculpture (36 archived evaluations)")
    );
    assert!(
        json["action"]
            .as_str()
...
fn independent_lane_status_summarizes_ready_lanes() {
    Command::cargo_bin("kryptos-k4")
        ...
        .success()
        .stdout(predicate::str::contains("Independent Lane Status"))
        .stdout(predicate::str::contains("lanes: 82"))
        .stdout(predicate::str::contains(
            "ready for source-backed observations: 82",
        ))
        .stdout(predicate::str::contains("invalid lanes: 0"))
        .stdout(predicate::str::contains("prediction artifacts: 82"))
        .stdout(predicate::str::contains("unique prediction artifacts: 40"))
        .stdout(predicate::str::contains(
            "unique ready prediction artifacts: 40",
        ))
    ...
    let output = Command::cargo_bin("kryptos-k4")
        ...
        .clone();
    let json: Value = serde_json::from_slice(&output).unwrap();
    assert_eq!(json["lane_count"], 82);
    assert_eq!(json["ready_for_source_backed_observations"], 82);
    assert_eq!(json["invalid_lanes"], 0);
    assert_eq!(json["prediction_artifacts"], 82);
    assert_eq!(json["unique_prediction_artifacts"], 40);
    assert_eq!(json["unique_ready_prediction_artifacts"], 40);
...
fn next_evidence_gate_prints_operational_checklist() {
    Command::cargo_bin("kryptos-k4")
        ...
        .success()
        .stdout(predicate::str::contains("Next Evidence Gate"))
        .stdout(predicate::str::contains("total ready lanes: 82"))
        .stdout(predicate::str::contains(
            "ciphertext-adjacent-contrast-position-prior",
        ))
        .stdout(predicate::str::contains(
